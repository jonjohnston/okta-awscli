...
class OktaAuth():
    ...
    def __init__(self):
        ...
        self._mfa_code = None
        self.https_base_url = 'https://veevasys.okta.com'
        self.username = input("Enter okta username : ")
    ...
    def primary_auth(self):
        ...
        if 'status' in resp_json:
            if resp_json['status'] == 'MFA_REQUIRED':
                ...
                print("""MFA not enrolled. Cannot continue.
Please enroll an MFA factor in the Okta Web UI first!""")
    ...
    def verify_mfa(self, factors_list, state_token):
        ...
        for factor in factors_list:
            if factor['factorType'] in supported_factor_types:
                ...
                print("Unsupported factorType: %s" %
                                 (factor['factorType'],))
    ...
    def get_apps(self, session_id):
        ...
        aws_apps = []
        for app in resp:
            if app['appName'] == "veevasystems_alibaba_1" or app['appName'] == "veevasystems_alibabacloud_1" or app['appName'] == "veevasystems_veevaalibabacloudprod_1":
                aws_apps.append(app)
        if not aws_apps:
            print("No Alibaba apps are available for your user. \
                Exiting.")
            sys.exit(1)
        ...
        if app_choice is None:
            ...
            app_choice = int(input('Please select Alibaba app: ')) - 1
        print("Selected app: %s" % aws_apps[app_choice]['label'])
    ...
    def _next_login_step(self, state_token, login_data):
        ...
        if status == 'UNAUTHENTICATED':
            print("You are not authenticated -- please try to log in again")
            exit(2)
...
#AUTH
class AliAuth():
    """ Methods to support Aliyun authentication using STS """

    def __init__(self):
        home_dir = os.path.expanduser('~')
        self.creds_dir = home_dir + "/.aliyun/"
        self.creds_file = self.creds_dir + "/config.json"
        self.role = ""
        parser = RawConfigParser()

    def choose_ali_role(self, assertion):
        """ Choose Aliyun role from SAML assertion """

        roles = self.__extract_available_roles_from(assertion)
        if self.role:
            predefined_role = self.__find_predefined_role_from(roles)
            if predefined_role:
                print("Using predefined role: %s" % self.role)
                return predefined_role
            else:
                print("""Predefined role, %s, not found in the list
of roles assigned to you.""" % self.role)
        ...
        role_choice = int(input('Please select the Aliyun role: ')) - 1
        return roles[role_choice]

    @staticmethod
    def __extract_available_roles_from(assertion):
        ali_attribute_role = 'https://www.aliyun.com/SAML-Role/Attributes/Role'
        attribute_value_urn = '{urn:oasis:names:tc:SAML:2.0:assertion}AttributeValue'
        roles = []
        role_tuple = namedtuple("RoleTuple", ["role_arn", "principal_arn"])
        root = ET.fromstring(base64.b64decode(assertion))
        for saml2attribute in root.iter('{urn:oasis:names:tc:SAML:2.0:assertion}Attribute'):
            if saml2attribute.get('Name') == ali_attribute_role:
...
role = ali_auth.choose_ali_role(assertion)
role_arn, principal_arn = role
f = open('/tmp/aliyunsaml', "w+")
f.write(principal_arn + ' ' + role_arn + ' ' + assertion)
f.close()
